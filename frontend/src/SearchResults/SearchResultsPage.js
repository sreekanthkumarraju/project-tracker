...
function SearchResultsPage(props) {
    ...
    async function fetchProjectCount() {
      if (loggedUser) {
        const projectCountResult = await fetch(
          `${server}/searchProjects/${loggedUser._id}/count/${query}`,
          {
            method: "GET",
          }
        );
  ...
  useEffect(() => {
    async function fetchProjectData() {
      if (loggedUser && loggedUser._id && query) {
        const dataResult = await fetch(
          `${server}/searchProjects/${loggedUser._id}/${query}/page/${page}`,
          {
            method: "GET",
          }
        );
